...
pub fn get_solver_status(db_path: &Path, solver_in_process: bool) -> SolverStatusPayload {
    ...
    if !db_available {
        return SolverStatusPayload {
            ...
            eta_seconds: None,
            solver_running: solver_in_process,
            status: if solver_in_process {
                "en_cours".into()
            } else {
                "pause".into()
            },
    ...
    let (db_solved, db_queued, db_phase, db_started, db_running_flag, db_updated, db_target, _db_progress) =
        if let Some(r) = row {
            (
                r.0.unwrap_or(0),
                r.1.unwrap_or(0),
    ...
    let running = solver_in_process || db_running_flag || workers;
    ...
    let status = if running {
        if stale_age.is_some_and(|a| a > 60.0) {
            "calcul_long"
        } else {
            "en_cours"
        }
...
pub fn get_process_status(solver_in_process: bool) -> ProcessStatusPayload {
    let running = if solver_in_process {
        true
    } else {
