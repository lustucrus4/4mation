...
function selfPlayWr(m: { self_play_win_rate_p1?: number; self_play_win_rate?: number }): number | null | undefined {
  return m.self_play_win_rate_p1 ?? m.self_play_win_rate;
}
...
export default function RlTrainingPage() {
  ...
  const metricsQ = useQuery({
    ...
    refetchInterval: (q) => (statusQ.data?.running ? 3000 : 10000),
  ...
  const evalsL5 = metrics.filter(

    (m) => (m.event === "eval_level5" || m.event === "eval_level_5") && m.eval_vs_level5 != null

  );

  const evalsL3 = metrics.filter(

    (m) => (m.event === "eval_level3" || m.event === "eval_level_3") && m.eval_vs_level3 != null

  );



  const winRates = selfPlay.map((m) => selfPlayWr(m) ?? 0);

  const evalRatesL5 = evalsL5.map((m) => m.eval_vs_level5 ?? 0);

  const evalRatesL3 = evalsL3.map((m) => m.eval_vs_level3 ?? 0);
    ...
    <div className="space-y-8">
      ...
      <header>
        ...
        <p className="mt-2 max-w-2xl text-white/60">
          ...
          AlphaZero-lite · MLP + MCTS · curriculum · éval périodique vs level_3 (intermédiaire) et
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="vs level_3" value={pct(status?.last_eval_vs_level3)} accent />

        <StatCard label="vs level_5" value={pct(status?.last_eval_vs_level5)} />
      ...
      {statusQ.isError && (
        ...
        <p className="rounded-lg border border-p1/40 bg-p1/10 px-4 py-3 text-sm text-p1">

          API indisponible — lancez l&apos;API depuis <code className="text-white/80">4mation/</code>{" "}

          (<code className="text-white/80">py api/app.py</code>) avec le front en{" "}
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <ChartBlock title="Self-play (J1)" subtitle={`${winRates.length} points`}>
        ...
        <ChartBlock title="vs level_3 (progression)" subtitle={`${evalRatesL3.length} évaluations`}>

          <Sparkline values={evalRatesL3} color="#a78bfa" />

          {evalRatesL3.length > 0 && (

            <MiniBar value={evalRatesL3[evalRatesL3.length - 1] ?? 0} max={1} color="#a78bfa" />

          )}

        </ChartBlock>

        <ChartBlock title="vs level_5 (expert)" subtitle={`${evalRatesL5.length} évaluations`}>

          <Sparkline values={evalRatesL5} color="#f97316" />

          {evalRatesL5.length > 0 && (

            <MiniBar value={evalRatesL5[evalRatesL5.length - 1] ?? 0} max={1} color="#f97316" />
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <thead className="bg-white/5 text-white/50">
              ...
              <tr>
                ...
                <th className="px-3 py-2">vs L3</th>

                <th className="px-3 py-2">vs L5</th>
            ...
            <tbody>
              ...
              {[...metrics].reverse().slice(0, 24).map((m, i) => (
                ...
                <tr key={`${m.step}-${m.event}-${i}`} className="border-t border-white/5">
                  ...
                  <td className="px-3 py-2">{pct(selfPlayWr(m))}</td>

                  <td className="px-3 py-2">
                    {m.event.startsWith("eval_level") ? pct(m.eval_vs_level3) : "—"}
                  </td>

                  <td className="px-3 py-2">
                    {m.event.startsWith("eval_level") ? pct(m.eval_vs_level5) : "—"}
                  </td>
...
.\\scripts\\rl.ps1 train -Fresh   # nouveau réseau AlphaZero-lite
