...
def audit(
    ...
    """Analyse chaque coup du bot suspect et mesure l'écart au meilleur coup."""
    findings: List[Dict[str, Any]] = []
    for entry in game["moves"]:
        if entry["bot"] != suspect:
            continue
...
def report(game: Dict[str, Any], findings: List[Dict[str, Any]], suspect: str) -> None:
    print(
    ...
    print(f"Audit des coups de {suspect}")
    counts: Dict[str, int] = {}
    for f in findings:
        counts[f["kind"]] = counts.get(f["kind"], 0) + 1
    total = sum(counts.values()) or 1
    for label, _ in SEUILS:
        n = counts.get(label, 0)
        print(f"  {label:<13} {n:>3} / {total}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--suspect",
        default=None,
        help="Bot audité (défaut : celui qui a les pierres O)",
    )
    ...
    suspect = args.suspect or args.second
    ...
    lookup = TablebaseLookup()
    findings = audit(game, suspect, lookup, args.depth, args.time_ms)
    report(game, findings, suspect)

    if args.json:
        payload = {"game": game, "findings": findings, "suspect": suspect}
